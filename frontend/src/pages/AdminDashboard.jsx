...
function cleanMessage(msg) {
  ...
  return BOILERPLATE_PHRASES.some(p => lower.includes(p)) ? 'Pending admin review' : msg
}

function getToken() {
  try {
    const user = JSON.parse(localStorage.getItem(STORAGE_KEY))
    // Match the shape the backend writes: { id: "...", ... }
...
function AdminDashboard() {
  ...
  const [recent,  setRecent]  = useState({ donations: [], requests: [] })
  ...
  useEffect(() => {
    async function loadDashboard() {
      ...
      try {
        ...
        if (!statsRes.ok || !donationsRes.ok || !requestsRes.ok) {
          const failedStatus = [statsRes, donationsRes, requestsRes]
            .find(r => !r.ok)?.status
          throw new Error(`Server returned ${failedStatus}`)
        }

        const [statsData, donationsData, requestsData] = await Promise.all([
          statsRes.json(),
          donationsRes.json(),
          requestsRes.json(),
        ])
        ...
        setStats(statsData || {})
        setRecent({
          donations: Array.isArray(donationsData) ? donationsData : donationsData?.donations || [],
          requests:  Array.isArray(requestsData)  ? requestsData  : requestsData?.requests  || [],
        })
      } catch (err) {
  ...
  if (error)   return <div className="ap-loading" style={{ color: '#dc2626' }}>{error}</div>
    ...
    <div>
      ...
      <div className="ap-card">
        <div className="ap-card-header">
          ...
          <Link to="/admin/donations" style={{ fontSize: 13, color: '#2563eb', textDecoration: 'none' }}>
            View all →
          </Link>
        </div>
          ...
          <table className="ap-table">
            ...
            <tbody>
              {recent.donations.length === 0 ? (
                <tr><td colSpan={4} className="ap-empty">No donations yet.</td></tr>
              ) : recent.donations.map(d => (
                <tr key={d.id || d._id}>
                  <td>{d.title || '—'}</td>
                  <td>
                    <span style={{ marginRight: 4 }}>{CATEGORY_ICONS[d.category] || '📦'}</span>
                    {d.category || '—'}
                  </td>
                  <td><span className={statusBadge(d.status)}>{d.status || 'unknown'}</span></td>
      ...
      <div className="ap-card">
        <div className="ap-card-header">
          ...
          <Link to="/admin/requests" style={{ fontSize: 13, color: '#2563eb', textDecoration: 'none' }}>
            View all →
          </Link>
        </div>
        <div className="ap-table-wrap">
          <table className="ap-table">
            ...
            <tbody>
              {recent.requests.length === 0 ? (
                <tr><td colSpan={4} className="ap-empty">No requests yet.</td></tr>
              ) : recent.requests.map(r => (
                <tr key={r.id || r._id}>
                  <td>
                    <span style={{ marginRight: 4 }}>{CATEGORY_ICONS[r.category] || '📦'}</span>
                    {r.category || '—'}
                  </td>
                  <td><span className={statusBadge(r.status)}>{r.status || 'unknown'}</span></td>
